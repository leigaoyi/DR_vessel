...
class CollectData:
    ...
    def reload(self,groundtruth,probgraph):
        ...
        self.groundtruth = sorted(self.groundtruth, key=lambda x: os.path.basename(x)[:2])
        self.probgraph = sorted(self.probgraph, key=lambda x: os.path.basename(x)[:2])
